...
func (c *Collider) Run(p int, useTls bool) {
	...
	http.HandleFunc("/getRoomCnt", c.getRoomClientCnt)
...
func (c *Collider) getRoomClientCnt(w http.ResponseWriter, r *http.Request) {
	w.Header().Add("Access-Control-Allow-Origin", "*")
	w.Header().Add("Access-Control-Allow-Methods", "GET")
	value := r.FormValue("roomId")
	count := 0
	if value != "" {
		if r, ok := c.rooms[value]; ok {
			count = len(r.clients)
		}
	}
	res := struct {
...
func (c *Collider) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	s := websocket.Server{Handler: c.wsHandler, Handshake: nil}
	s.ServeHTTP(w, req)
}
